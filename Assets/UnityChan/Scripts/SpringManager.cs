...
namespace UnityChan
{
	public class SpringManager : MonoBehaviour
	{
		...
		public void ResetAllSprings()
		{
            ...
            {
                pendingReset = false;
                // LateUpdate の先頭で処理することで Animator 評価後の最終ボーン位置を使う。
                // レスト姿勢にそろえてから child.position をスナップし、100回収束させる。
                for (int i = 0; i < springBones.Length; i++)
                    springBones[i].ResetToRestPose();
                for (int i = 0; i < springBones.Length; i++)
                    springBones[i].ResetSpring();
                for (int j = 0; j < 100; ++j)
                    for (int i = 0; i < springBones.Length; i++)
                        if (dynamicRatio > springBones[i].threshold)
                            springBones[i].UpdateSpring();
                return; // 収束済みなので通常の UpdateSpring はスキップ
            }
        }
